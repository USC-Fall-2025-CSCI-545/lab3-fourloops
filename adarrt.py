...
class AdaRRT():
    ...
    def _get_random_sample(self):
        ...
        :returns: A vector representing a randomly sampled point in the search
            space.
        """
        # uniformly sample the space
        sample=np.random.uniform(self.joint_lower_limits,self.joint_upper_limits)
        return sample
    ...
    def _get_nearest_neighbor(self, sample):
        ...
        """
        # calculate all the node distance and find the minimum distance and return the node
        all_nodes=list(self.start)
        distances=[]
        filter_node=[]
    ...
    def _extend_sample(self, sample, neighbor):
        ...
        # check whether the node will collide or not
        if distance==0:
            return None
        norm_direction=direction/distance
        new_state=neighbor.state+norm_direction*self.step_size
        if not self._check_for_collision(new_state):
            # if no collision then add this node
            new=AdaRRT.Node(new_state,neighbor)
            # new.parent=neighbor
            neighbor.children.append(new)
    ...
    def _check_for_completion(self, node):
        ...
        """
        # calculate the distance of the node and the goal
        # if within the self.goal_precision distance of the goal return True, otherwise False
        dis=node.state-self.goal.state
    ...
    def _trace_path_from_start(self, node=None):
        ...
        """
        # if no node input, use default node self.goal
        if node is None:
            node=self.goal
        ...
        while current_node is not None:
            trace_path.append(current_node.state)
            current_node=current_node.parent
        
        # reverse the trace path
        trace_path.reverse()
        return trace_path
    ...
    # get the random sample near the goal
    def _get_random_sample_near_goal(self, radius=0.05):
        # get the size
        dimension=len(self.goal.state)
        # get the offset for each axis
        offset=np.random.uniform(-radius,radius,dimension)
        sample=self.goal.state+offset

        # generate the sample
        random_sample=np.clip(sample,self.joint_lower_limits,self.joint_upper_limits)
